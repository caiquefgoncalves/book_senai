...
import {editarLivro} from "../services/livros/editarLivro";
...
export default function EditarLivro({ route, navigation }) {
    ...
    useEffect(() => {
        buscarLivroPorId(livroId, (livro) => {
            if (livro) {
                ...
                setFaixaEtaria(livro.faixa_etaria || '');
            }
        });
    }, [livroId]);
    ...
    const handleEditar = async () => {
        ...
        try {
            await editarLivro(livroId, imagem, titulo, categoria, descricao, autor, faixaEtaria);
            Alert.alert('Sucesso', 'Livro editado!', [
                {
                    text: 'OK',
